...
namespace Automanager.Resource
{
    public static class CultureHelper
    {
        ...
        public static List<string> SupportedLanguages()
        {
            try
            {
                return ConfigurationManager.AppSettings["SupportedLanguages"].Split(',').ToList();
            }
            catch
            {
                return new List<string> { GetDefaultCulture() };
            }
        }

        /// <summary>
        /// Returns a valid culture name based on "name" parameter. If "name" is not valid, it returns the default culture got from web config
        /// </summary>
        ...
        public static string GetImplementedCulture(string name)
        {
            ...
            if (!ValidCultures.Any(c => c.Trim().Equals(name, StringComparison.InvariantCultureIgnoreCase)))
                return GetDefaultCulture();

            // if it is implemented, accept it
            if (Cultures.Any(c => c.Trim().Equals(name, StringComparison.InvariantCultureIgnoreCase)))
        ...
        public static CultureInfo GetParent(this CultureInfo culture)
        {
            CultureInfo cultureParent = culture;
            do
            {
                cultureParent = cultureParent.Parent;
            } while (!string.IsNullOrEmpty(cultureParent.Parent.Name));
            return cultureParent;
        }
    }
}
